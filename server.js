...
const cors = require("cors"); // ✅ Added for cross-origin (Speech-to-text)
const session = require("express-session"); // ✅ Optional: for greeting after login
...
// ✅ Optional: enable sessions (to greet logged-in users)
app.use(session({
  secret: "women-safety-secret-key",
  resave: false,
...
// ✅ NEW TABLE for Speech-to-Text
db.query(`
CREATE TABLE IF NOT EXISTS trancrip (
  id INT AUTO_INCREMENT PRIMARY KEY,
  content TEXT,
  ...
  try {
    ...
    db.query(
      ...
      (err, result) => {
        ...
        req.session.username = firstname; // ✅ store in session
...
app.post("/login", (req, res) => {
  ...
  db.query("SELECT * FROM users WHERE email = ?", [email], async (err, results) => {
    ...
    req.session.username = user.firstname; // ✅ store in session
    res.redirect(`/index.html?name=${encodeURIComponent(user.firstname)}`);
...
app.get("/greet", (req, res) => {
  if (req.session.username) {
    res.send(`Hi ${req.session.username}, welcome back!`);
  } else {
    res.send("Welcome! Please log in first.");
  }
});
...
// ✅ ---- SPEECH-TO-TEXT SAVE API ----
app.post("/save", (req, res) => {
  console.log("📩 Received POST request at /save");
  const { text } = req.body;

  if (!text || text.trim() === "") {
    return res.status(400).json({ error: "Empty text cannot be saved." });
  }

  const sql = "INSERT INTO trancrip (content) VALUES (?)";
